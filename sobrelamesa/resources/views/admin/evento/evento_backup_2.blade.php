...
$('#tbl_venta').sortable({
    ...
    update: function (event, ui) {
            ...
            for (var j = 0; j < list_venta.length; j++) {
                if(result[i].includes('row-')){
                    if(list_venta[j].key == parseInt(result[i].replace('row-', ''))){
                        //console.log("list_venta[j].key "+list_venta[j].key+" parseInt(result[i].replace('row-', '')) "+parseInt(result[i].replace('row-', '')));
                        //list_venta[j].key = pos_list_venta;
        ...
        console.log("data "+data);
        console.log("result 02 ------- " + result);*/
        //swap(list_venta, ui.item.index() -1 , list_venta[parseInt(ui.item[0].id.replace('row-', ''))].row_position);
        //console.log(ui.item);
...
/*
        ...
        console.log(" row_position "+list_venta[parseInt(ui.item[0].id.replace('row-', ''))].row_position);
        swap(list_venta,  list_venta[parseInt(ui.item[0].id.replace('row-', ''))].row_position, ui.item.index());
        ...
        */
        ...
        //$("#"+ui.item[0].id).attr("id", ui.item.index());
        //list_venta[parseInt(ui.item[0].id.replace('row-', ''))].row_position = ui.item.index() -1;
        //list_venta.sort((a, b) => b.row_position - a.row_position);
        ...
        console.log(list_venta);
        console.log(ui.item[0].id);
        console.log(parseInt(ui.item[0].id.replace('row-', '')));
        console.log(ui.item.index());
        swap(list_venta, parseInt(ui.item[0].id.replace('row-', '')), ui.item.index());
        console.log(list_venta);
...
function swap(arr, index1, index2){
    //console.log(JSON.stringify(arr)+" "+index1+" "+index2);
    //console.log(arr[index1].row_position+" "+arr[index2].row_position);
    /*let aux = arr[index1].row_position;
    arr[index1].row_position = arr[index2].row_position;
    arr[index2].row_position = aux;*/

    //console.log(JSON.stringify(arr)+" "+index1+" "+index2);
    let aux = arr[index1].row_position;//guardo el 4
    arr[index1].row_position = index2;// 4 se  asigno 2
    arr[index2].row_position = aux;// y a 2 le asigno 4
