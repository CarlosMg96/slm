                                ...
                                <tr>
                                    ...
                                    <?php
                                        ...
                                        for ($i=1; $i <= $dias_mes; $i++) { 
                                            echo '<th>'.$i.'/'.$mes_actual.'/'.$anio_actual.'</th>';
                                            foreach ($objectEventosPorMes as $key => $evento) {
                                                //if (date("Y-m-d", strtotime($evento->fecha_evento)) == date("Y-m-d", strtotime($anio_actual.'-'.$mes_actual.'-'.$i))) {
                                                if(check_in_range($evento->fecha_entrega, $evento->fecha_recoleccion, $anio_actual.'-'.$mes_actual.'-'.$i)){
                                                    echo '<th>#00'.$evento->id.'</th>';                                       
                                                }
                                            }
                                        }
                                    ?>                                                     
                                ...
                                foreach ($objectProducto as $key => $producto) {
                                    ...
                                    for ($i=1; $i <= $dias_mes; $i++) {
                                        ...
                                        foreach ($objectEventosPorMes as $key => $evento) {
                                            //if (date("Y-m-d", strtotime($evento->fecha_evento)) == date("Y-m-d", strtotime($anio_actual.'-'.$mes_actual.'-'.$i))) {
                                            if(check_in_range($evento->fecha_entrega, $evento->fecha_recoleccion, $anio_actual.'-'.$mes_actual.'-'.$i)){
                                                foreach ($evento->detalle_evento as $key => $evento_producto) {
                                                    if ($producto->id == $evento_producto->producto_id) {
                                                        echo '<td>'.$evento_producto->cantidad.'</td>';
                                                    }
                                                }
                                            }
                                        }
...
<script type="text/javascript">
    $('#menu_inventario').attr('class','nav-link active');
	var listCategorias = [];
